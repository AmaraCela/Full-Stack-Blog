    ...
    return ( 
        <div className="flex justify-center h-full items-center">
            <div className={`${formProp.height} login-div flex rounded-md`}>
                <form className="bg-[#ffffff] grid justify-evenly p-4 items-center rounded-l-md login-form" method="POST">
                    ...
                    {formProp.inputs.map((input)=>(
                        ...
                        <input type={input.type} key={input.id} required placeholder={input.placeholder} className="input-format" name={input.id}/></div>
                    ))}
